# TheRock-CI-Config PR Bot — Policy FAQ Doc

**TheRock-CI-Config PR Bot** is an automated Pull Request (PR) gatekeeper.
...
These run as separate CI workflows. The bot does **not** wait for them — it
takes a snapshot and shows each as ✅ / ❌ / ⏳ Pending in the same table, then
exits immediately. The table refreshes automatically when those workflows
finish. They are enforced by **branch protection** (required status checks),
not by the PR Bot.

**The bot shows ⏳ Pending — did it time out?**

No. The bot has **no timeout** and never blocks on other checks, so it cannot
fail because CI was slow. A ⏳ row simply means that workflow had not concluded
when the table was last written; it updates on its own once the check finishes.
