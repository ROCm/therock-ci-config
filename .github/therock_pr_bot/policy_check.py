...
def resolve_pr_number_from_sha(
    owner: str, repo: str, sha: str, token: str
) -> Optional[int]:
    """Find the open PR whose HEAD is `sha`.

    On `check_run` / `check_suite` events GitHub frequently returns an empty
    `pull_requests` array, so PR_NUMBER may be unavailable. This resolves it
    from the commit SHA via `GET /commits/{sha}/pulls`, preferring an open PR
    whose head SHA matches exactly.
    """
    if not sha:
        return None
    try:
        data = gh_get(
            f"https://api.github.com/repos/{owner}/{repo}/commits/{sha}/pulls?per_page=100",
            token,
        )
    except RuntimeError as exc:
        print(f"⚠️  Could not resolve PR from SHA {sha}: {exc}", file=sys.stderr)
        return None
    if not isinstance(data, list) or not data:
        return None
    # Prefer an OPEN PR whose head SHA matches exactly.
    for pr in data:
        if not isinstance(pr, dict):
            continue
        if pr.get("state") == "open" and str(
            (pr.get("head") or {}).get("sha") or ""
        ) == str(sha):
            return int(pr.get("number"))
    # Fall back to the first open PR, then the first PR.
    for pr in data:
        if isinstance(pr, dict) and pr.get("state") == "open":
            return int(pr.get("number"))
    first = data[0]
    return int(first.get("number")) if isinstance(first, dict) else None
...
def build_policy_table_comment(
    ...
    override_url = (
        "https://github.com/ROCm/therock-ci-config/tree/main/docs/"
        "THE_ROCK_CI_CONFIG_FAQ.md#-wish-to-override-the-policy-process-and-get-unblocked"
    )
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=300,
        help="Max seconds to wait for required checks (e.g. pre-commit) to "
        "conclude before rendering their last-known state. NEVER fails on "
        "timeout — it only refreshes the results table.",
    )
    parser.add_argument(
        "--poll-seconds",
        type=int,
        default=15,
        help="Seconds between table refreshes while required checks run.",
    )
    ...
    # On check_run / check_suite events the PR number is often absent — resolve
    # it from the commit SHA so the results table can still refresh.
    if (
        (not pr_number_s or not pr_number_s.strip())
        and token
        and owner
        and repo
        and sha
    ):
        resolved = resolve_pr_number_from_sha(owner, repo, sha, token)
        if resolved is not None:
            pr_number_s = str(resolved)
            print(f"ℹ️  Resolved PR #{pr_number_s} from SHA {sha}.")
    ...
    # Snapshot the OTHER required checks once. We do NOT wait for them:
    # waiting burns a runner and turns a slow/queued check into a false
    # "timeout" failure. They are shown as ⏳ Pending purely for information and
    # are enforced by branch protection (required status checks), not by us.
    runs = get_check_runs(owner=owner, repo=repo, sha=sha, token=token)  # type: ignore[arg-type]
    _, failing, _ = summarize_required_checks(policy, runs)

    ready = not errors and not failing
    combined = results + build_check_results(policy, runs, include_self=ready)
    ...
    upsert_comment(
        ...
        token,  # type: ignore[arg-type]
        marker,
        build_policy_table_comment(combined, marker, ready=ready),
    )

    # Label depends ONLY on our own policy (missing JIRA/ISSUE reference),
    # never on whether another workflow has finished yet.
    ...
    print("✅ Policy checks passed (other required checks tracked separately).")
    return 0
